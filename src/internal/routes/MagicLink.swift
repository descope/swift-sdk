...
class MagicLink: DescopeMagicLink {
    ...
    func signIn(with method: DeliveryMethod, loginId: String, uri: String?, options: [SignInOptions]) async throws -> String {
        let (refreshJwt, loginOptions) = try options.convert()
        return try await client.magicLinkSignIn(with: method, loginId: loginId, uri: uri, refreshJwt: refreshJwt, options: loginOptions).convert(method: method)
    }
    
    func signUpOrIn(with method: DeliveryMethod, loginId: String, uri: String?, options: [SignInOptions]) async throws -> String {
        let (refreshJwt, loginOptions) = try options.convert()
        return try await client.magicLinkSignUpOrIn(with: method, loginId: loginId, uri: uri, refreshJwt: refreshJwt, options: loginOptions).convert(method: method)
    }
