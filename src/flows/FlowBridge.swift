...
extension FlowBridge {
    func evaluateJavaScript(_ code: String) async -> Any? {
        return try? await webView?.evaluateJavaScript(anonymousFunction(body: code))
    }

    func runJavaScript(_ code: String) {
        webView?.evaluateJavaScript(anonymousFunction(body: code))
    }
