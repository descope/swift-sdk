...
extension DescopeSessionManager {
    /// The state of a `DescopeSessionManager` object.
    public enum State {
        /// The state of the session manager when there's no active session, usually
        /// because the user hasn't signed in to the host application yet.
        case empty
        
        /// The manager has a valid `DescopeSession` and its session JWT can be used
        /// to authenticate outgoing server requests.
        case valid
        
        /// The `DescopeSession`'s session JWT is no longer valid but the refresh JWT
        /// can be used to get a new one.
        case sessionExpired
        
        /// The `DescopeSession`'s refresh JWT is no longer valid and the application
        /// should most likely ask the user to sign in again.
        case refreshExpired
    }
    
    /// The current state of this `DescopeSessionManager` object.
    ///
    /// Using this property might be more convenient in some circumstances instead of
    /// ensuring that `session` is not `nil`.
    ///
    ///     if Descope.sessionManager.state == .refreshExpired {
    ///         present(LogoutViewController(), animated: true)
    ///     }
